...
class Agent():
    ...
    def learn(self):
        ...
        output = self.model.predict(np.array(states))

        next_state_values = self.model_target.predict(np.array(next_states))

        for i in range(32):
            action = self.possible_actions.index(actions_taken[i])
            output[i][action] = next_rewards[i] + (not next_done_flags[i]) * self.gamma * max(next_state_values[i])


        self.model.fit(np.array(states),output,batch_size = 32, epochs = 1, verbose = 0)
